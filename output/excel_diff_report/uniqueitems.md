# Differences for uniqueitems.txt

*Key column used: `index`*

## Added Columns in BK (New)
`disabled, spawnable, disableChronicle, DropConditionCalc`  

## Removed Columns in BK (New)
`enabled`  

## Added Rows in BK (New) (40)
- ars al'diablolos
- ars dul'mephistos
- ars tor'baalos
- black cleft
- bloodpact shard
- bone break
- charm blank
- cirilkas guidance
- cold rupture
- crack of the heavens
- crafted black cleft
- crafted bone break
- crafted cold rupture
- crafted crack of the heavens
- crafted flame rift
- crafted rotting fissure
- defender's bile
- defender's fire
- dreadfang
- entropy locket
- flame rift
- gheed's wager
- guardian's light
- guardian's thunder
- measured wrath
- opalvein
- precrafted black cleft
- precrafted bone break
- precrafted cold rupture
- precrafted crack of the heavens
- precrafted flame rift
- precrafted rotting fissure
- protector's frost
- protector's stone
- rotting fissure
- sling
- titan's echo
- unique warlock helm
- warlock class pack
- wraithstep

## Removed Rows in BK (New) (2)
- ciris's guidance
- ravaging t1

## Modified Rows (91)
### amazon's teaching
- `*ID`: $\color{gray}{\text{421}}$ (Old) &rarr; $\color{blue}{\text{459}}$ (New)
- `prop7`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{pierce-dmg}}$ (New)
- `min7`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{4}}$ (New)
- `max7`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)

### amulet of the viper
- `min1`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max1`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `prop2`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{res-pois}}$ (New)
- `min2`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{25}}$ (New)
- `max2`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{25}}$ (New)
- `min3`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max3`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)

### annihilus
- `max2`: $\color{gray}{\text{25}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)
- `max3`: $\color{gray}{\text{25}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)
- `max4`: $\color{gray}{\text{25}}$ (Old) &rarr; $\color{blue}{\text{15}}$ (New)

### arcbane
- `*ID`: $\color{gray}{\text{448}}$ (Old) &rarr; $\color{blue}{\text{486}}$ (New)
- `firstLadderSeason`: $\color{gray}{\text{Magic Conviction}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `par1`: $\color{gray}{\text{376}}$ (Old) &rarr; $\color{blue}{\text{431}}$ (New)
- `min1`: $\color{gray}{\text{3}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)

### arkaine's valor
- `max7`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### azurewrath
- `min4`: $\color{gray}{\text{500}}$ (Old) &rarr; $\color{blue}{\text{250}}$ (New)

### baal's heart
- `*ID`: $\color{gray}{\text{409}}$ (Old) &rarr; $\color{blue}{\text{447}}$ (New)
- `prop2`: $\color{gray}{\text{Aura}}$ (Old) &rarr; $\color{blue}{\text{aura}}$ (New)

### bloodraven's charge
- `min3`: $\color{gray}{\text{13}}$ (Old) &rarr; $\color{blue}{\text{30}}$ (New)
- `max3`: $\color{gray}{\text{13}}$ (Old) &rarr; $\color{blue}{\text{30}}$ (New)

### bul katho's mind
- `*ID`: $\color{gray}{\text{410}}$ (Old) &rarr; $\color{blue}{\text{448}}$ (New)
- `prop8`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{pierce-dmg}}$ (New)
- `min8`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `max8`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)

### calamity
- `*ID`: $\color{gray}{\text{440}}$ (Old) &rarr; $\color{blue}{\text{478}}$ (New)

### cerebus
- `prop6`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{pierce-dmg}}$ (New)
- `min6`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `max6`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)

### charm level reward
- `*ID`: $\color{gray}{\text{405}}$ (Old) &rarr; $\color{blue}{\text{439}}$ (New)
- `carry1`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `cost mult`: $\color{gray}{\text{3}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)
- `cost add`: $\color{gray}{\text{5000}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)

### charm modifiers
- `*ID`: $\color{gray}{\text{401}}$ (Old) &rarr; $\color{blue}{\text{438}}$ (New)
- `carry1`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `cost mult`: $\color{gray}{\text{3}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)
- `cost add`: $\color{gray}{\text{5000}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)
- `par3`: $\color{gray}{\text{16}}$ (Old) &rarr; $\color{blue}{\text{4}}$ (New)
- `prop4`: $\color{gray}{\text{aura}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `par4`: $\color{gray}{\text{377}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min4`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `max4`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `prop5`: $\color{gray}{\text{addxp}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min5`: $\color{gray}{\text{-50}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `max5`: $\color{gray}{\text{-50}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)

### corrupted arkaine's valor
- `*ID`: $\color{gray}{\text{427}}$ (Old) &rarr; $\color{blue}{\text{465}}$ (New)

### corrupted azurewrath
- `*ID`: $\color{gray}{\text{430}}$ (Old) &rarr; $\color{blue}{\text{468}}$ (New)
- `prop7`: $\color{gray}{\text{mag\%}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min7`: $\color{gray}{\text{10}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `max7`: $\color{gray}{\text{15}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)

### corrupted bartuc's cut-throat
- `*ID`: $\color{gray}{\text{429}}$ (Old) &rarr; $\color{blue}{\text{467}}$ (New)

### corrupted harlequin crest
- `*ID`: $\color{gray}{\text{424}}$ (Old) &rarr; $\color{blue}{\text{462}}$ (New)

### corrupted saracen's chance
- `*ID`: $\color{gray}{\text{428}}$ (Old) &rarr; $\color{blue}{\text{466}}$ (New)

### corrupted valkyrie wing
- `*ID`: $\color{gray}{\text{425}}$ (Old) &rarr; $\color{blue}{\text{463}}$ (New)

### crown of ages
- `max4`: $\color{gray}{\text{150}}$ (Old) &rarr; $\color{blue}{\text{200}}$ (New)
- `max6`: $\color{gray}{\text{15}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)
- `max7`: $\color{gray}{\text{50}}$ (Old) &rarr; $\color{blue}{\text{100}}$ (New)
- `min8`: $\color{gray}{\text{12}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max8`: $\color{gray}{\text{15}}$ (Old) &rarr; $\color{blue}{\text{30}}$ (New)
- `min9`: $\color{gray}{\text{7}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `prop10`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{mana-kill}}$ (New)
- `min10`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `max10`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{9}}$ (New)

### deathcleaver
- `prop7`: $\color{gray}{\text{dmg-norm}}$ (Old) &rarr; $\color{blue}{\text{dmg}}$ (New)
- `min7`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{50}}$ (New)
- `prop8`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{pierce-dmg}}$ (New)
- `min8`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max8`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)

### destiny
- `*ID`: $\color{gray}{\text{445}}$ (Old) &rarr; $\color{blue}{\text{483}}$ (New)

### dimoaks hew
- `prop7`: $\color{gray}{\text{Vit}}$ (Old) &rarr; $\color{blue}{\text{vit}}$ (New)

### elemental exposure
- `*ID`: $\color{gray}{\text{437}}$ (Old) &rarr; $\color{blue}{\text{475}}$ (New)

### elemental union
- `*ID`: $\color{gray}{\text{417}}$ (Old) &rarr; $\color{blue}{\text{455}}$ (New)
- `prop1`: $\color{gray}{\text{Aura}}$ (Old) &rarr; $\color{blue}{\text{aura}}$ (New)

### gaia's precision
- `*ID`: $\color{gray}{\text{413}}$ (Old) &rarr; $\color{blue}{\text{451}}$ (New)
- `min1`: $\color{gray}{\text{3}}$ (Old) &rarr; $\color{blue}{\text{6}}$ (New)
- `max1`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `min6`: $\color{gray}{\text{200}}$ (Old) &rarr; $\color{blue}{\text{1000}}$ (New)
- `max6`: $\color{gray}{\text{300}}$ (Old) &rarr; $\color{blue}{\text{1000}}$ (New)
- `prop7`: $\color{gray}{\text{red-dmg}}$ (Old) &rarr; $\color{blue}{\text{hp\%}}$ (New)
- `min7`: $\color{gray}{\text{15}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `max7`: $\color{gray}{\text{20}}$ (Old) &rarr; $\color{blue}{\text{15}}$ (New)
- `prop9`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{hit-skill}}$ (New)
- `par9`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{Lightning Strike}}$ (New)
- `min9`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)
- `max9`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{40}}$ (New)

### giant's protection
- `*ID`: $\color{gray}{\text{412}}$ (Old) &rarr; $\color{blue}{\text{450}}$ (New)
- `max1`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### gimmershred
- `max4`: $\color{gray}{\text{511}}$ (Old) &rarr; $\color{blue}{\text{501}}$ (New)

### griffon's eye
- `prop6`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{oskill}}$ (New)
- `par6`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{Thunder Storm}}$ (New)
- `min6`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)
- `max6`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### guardian angel
- `max9`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### gungnir's almighty
- `*ID`: $\color{gray}{\text{411}}$ (Old) &rarr; $\color{blue}{\text{449}}$ (New)

### hell forge hammer
- `min4`: $\color{gray}{\text{66}}$ (Old) &rarr; $\color{blue}{\text{35}}$ (New)
- `max4`: $\color{gray}{\text{66}}$ (Old) &rarr; $\color{blue}{\text{35}}$ (New)
- `prop5`: $\color{gray}{\text{dmg-norm}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min5`: $\color{gray}{\text{66}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)
- `max5`: $\color{gray}{\text{66}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)

### hellfire torch
- `max1`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)
- `max2`: $\color{gray}{\text{25}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)
- `max3`: $\color{gray}{\text{25}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)

### homunculus
- `rarity`: $\color{gray}{\text{50}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)

### honor
- `*ID`: $\color{gray}{\text{444}}$ (Old) &rarr; $\color{blue}{\text{482}}$ (New)

### horadric staff
- `min1`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max1`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `prop2`: $\color{gray}{\text{res-all}}$ (Old) &rarr; $\color{blue}{\text{res-pois}}$ (New)
- `min2`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{25}}$ (New)
- `max2`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{25}}$ (New)
- `min3`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max3`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `prop4`: $\color{gray}{\text{cast3}}$ (Old) &rarr; $\color{blue}{\text{res-all}}$ (New)
- `min4`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max4`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `prop6`: $\color{gray}{\text{dmg-norm}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min6`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)
- `max6`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)

### jym's swift journey
- `*ID`: $\color{gray}{\text{431}}$ (Old) &rarr; $\color{blue}{\text{469}}$ (New)

### killing intent
- `*ID`: $\color{gray}{\text{436}}$ (Old) &rarr; $\color{blue}{\text{474}}$ (New)

### king's absolution
- `*ID`: $\color{gray}{\text{418}}$ (Old) &rarr; $\color{blue}{\text{456}}$ (New)
- `max2`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### kira's guardian
- `rarity`: $\color{gray}{\text{50}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)
- `max5`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### lidless wall
- `max2`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)

### mad dog
- `*ID`: $\color{gray}{\text{446}}$ (Old) &rarr; $\color{blue}{\text{484}}$ (New)

### manawyrm
- `*ID`: $\color{gray}{\text{449}}$ (Old) &rarr; $\color{blue}{\text{487}}$ (New)
- `firstLadderSeason`: $\color{gray}{\text{Magic Conviction}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `par1`: $\color{gray}{\text{376}}$ (Old) &rarr; $\color{blue}{\text{431}}$ (New)
- `par6`: $\color{gray}{\text{2}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `max6`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### mang song's lesson
- `min1`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)
- `max1`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)
- `min2`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `max2`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `prop3`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{hp\%}}$ (New)
- `min3`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{40}}$ (New)
- `max3`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{50}}$ (New)
- `max5`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{80}}$ (New)
- `min6`: $\color{gray}{\text{85}}$ (Old) &rarr; $\color{blue}{\text{115}}$ (New)
- `max6`: $\color{gray}{\text{85}}$ (Old) &rarr; $\color{blue}{\text{115}}$ (New)
- `max7`: $\color{gray}{\text{40}}$ (Old) &rarr; $\color{blue}{\text{50}}$ (New)

### medusa's gaze
- `max10`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)
- `par11`: $\color{gray}{\text{frost nova}}$ (Old) &rarr; $\color{blue}{\text{Frost Nova}}$ (New)

### meshif's travellers
- `*ID`: $\color{gray}{\text{408}}$ (Old) &rarr; $\color{blue}{\text{446}}$ (New)
- `prop5`: $\color{gray}{\text{Aura}}$ (Old) &rarr; $\color{blue}{\text{aura}}$ (New)
- `max8`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### mindnumb
- `*ID`: $\color{gray}{\text{451}}$ (Old) &rarr; $\color{blue}{\text{489}}$ (New)
- `firstLadderSeason`: $\color{gray}{\text{Magic Conviction}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `par1`: $\color{gray}{\text{376}}$ (Old) &rarr; $\color{blue}{\text{431}}$ (New)
- `prop5`: $\color{gray}{\text{cold-res}}$ (Old) &rarr; $\color{blue}{\text{res-cold}}$ (New)

### mistiltein
- `*ID`: $\color{gray}{\text{415}}$ (Old) &rarr; $\color{blue}{\text{453}}$ (New)
- `prop1`: $\color{gray}{\text{Aura}}$ (Old) &rarr; $\color{blue}{\text{aura}}$ (New)

### neverfrost
- `*ID`: $\color{gray}{\text{402}}$ (Old) &rarr; $\color{blue}{\text{441}}$ (New)

### nightwing's veil
- `max6`: $\color{gray}{\text{12}}$ (Old) &rarr; $\color{blue}{\text{15}}$ (New)

### numbness
- `*ID`: $\color{gray}{\text{442}}$ (Old) &rarr; $\color{blue}{\text{480}}$ (New)

### pride
- `*ID`: $\color{gray}{\text{443}}$ (Old) &rarr; $\color{blue}{\text{481}}$ (New)

### rainbow facet
- `*ID`: $\color{gray}{\text{404}}$ (Old) &rarr; $\color{blue}{\text{443}}$ (New)

### resistbreaker
- `*ID`: $\color{gray}{\text{447}}$ (Old) &rarr; $\color{blue}{\text{485}}$ (New)
- `firstLadderSeason`: $\color{gray}{\text{Magic Conviction}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `par1`: $\color{gray}{\text{376}}$ (Old) &rarr; $\color{blue}{\text{431}}$ (New)

### robe of the horadrim
- `*ID`: $\color{gray}{\text{438}}$ (Old) &rarr; $\color{blue}{\text{476}}$ (New)

### saboteurs blade
- `*ID`: $\color{gray}{\text{414}}$ (Old) &rarr; $\color{blue}{\text{452}}$ (New)
- `min2`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{2}}$ (New)

### saracen's chance
- `max4`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### schaefer's hammer
- `max1`: $\color{gray}{\text{20}}$ (Old) &rarr; $\color{blue}{\text{50}}$ (New)
- `prop9`: $\color{gray}{\text{dmg-ltng}}$ (Old) &rarr; $\color{blue}{\text{pierce-dmg}}$ (New)
- `min9`: $\color{gray}{\text{50}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max9`: $\color{gray}{\text{200}}$ (Old) &rarr; $\color{blue}{\text{15}}$ (New)

### shadowdancer
- `max5`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### sigurds staunching helm
- `*ID`: $\color{gray}{\text{419}}$ (Old) &rarr; $\color{blue}{\text{457}}$ (New)
- `max4`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### simple solution
- `*ID`: $\color{gray}{\text{422}}$ (Old) &rarr; $\color{blue}{\text{460}}$ (New)
- `prop8`: $\color{gray}{\text{res-poi-len}}$ (Old) &rarr; $\color{blue}{\text{res-pois-len}}$ (New)

### skin of the flayerd one
- `max9`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### skorn
- `*ID`: $\color{gray}{\text{439}}$ (Old) &rarr; $\color{blue}{\text{477}}$ (New)
- `min3`: $\color{gray}{\text{250}}$ (Old) &rarr; $\color{blue}{\text{180}}$ (New)
- `max3`: $\color{gray}{\text{300}}$ (Old) &rarr; $\color{blue}{\text{250}}$ (New)

### slanderer
- `*ID`: $\color{gray}{\text{441}}$ (Old) &rarr; $\color{blue}{\text{479}}$ (New)

### slayer's edge
- `*ID`: $\color{gray}{\text{420}}$ (Old) &rarr; $\color{blue}{\text{458}}$ (New)

### spike thorn
- `par10`: $\color{gray}{\text{Thorns}}$ (Old) &rarr; $\color{blue}{\text{thorns}}$ (New)

### spirit ward
- `max8`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### staff of kings
- `min1`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max1`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `prop3`: $\color{gray}{\text{cast3}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min3`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)
- `max3`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)
- `prop4`: $\color{gray}{\text{dmg-norm}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min4`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)
- `max4`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)

### static accumulator
- `*ID`: $\color{gray}{\text{433}}$ (Old) &rarr; $\color{blue}{\text{471}}$ (New)
- `prop1`: $\color{gray}{\text{sock}}$ (Old) &rarr; $\color{blue}{\text{gethit-skill}}$ (New)
- `par1`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{Static Field}}$ (New)
- `min1`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `max1`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)
- `prop2`: $\color{gray}{\text{enr\%}}$ (Old) &rarr; $\color{blue}{\text{move1}}$ (New)
- `min2`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `prop3`: $\color{gray}{\text{gethit-skill}}$ (Old) &rarr; $\color{blue}{\text{sock}}$ (New)
- `par3`: $\color{gray}{\text{Static Field}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min3`: $\color{gray}{\text{5}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)
- `max3`: $\color{gray}{\text{20}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)
- `prop4`: $\color{gray}{\text{move1}}$ (Old) &rarr; $\color{blue}{\text{enr\%}}$ (New)
- `min4`: $\color{gray}{\text{5}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)

### stealskull
- `max8`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{7}}$ (New)

### steel carapice
- `max2`: $\color{gray}{\text{20}}$ (Old) &rarr; $\color{blue}{\text{40}}$ (New)
- `min5`: $\color{gray}{\text{10}}$ (Old) &rarr; $\color{blue}{\text{50}}$ (New)
- `max5`: $\color{gray}{\text{15}}$ (Old) &rarr; $\color{blue}{\text{100}}$ (New)
- `prop10`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{allskills}}$ (New)
- `min10`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)
- `max10`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{2}}$ (New)
- `prop11`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{ease}}$ (New)
- `min11`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{-50}}$ (New)
- `max11`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{-50}}$ (New)

### steelpillar
- `max9`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### stone crusher
- `prop8`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{pierce-dmg}}$ (New)
- `min8`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `max8`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)

### stormspire
- `prop9`: $\color{gray}{\text{gethit-skill}}$ (Old) &rarr; $\color{blue}{\text{91}}$ (New)

### superkhalimflail
- `prop4`: $\color{gray}{\text{att\%}}$ (Old) &rarr; $\color{blue}{\text{att}}$ (New)
- `min4`: $\color{gray}{\text{200}}$ (Old) &rarr; $\color{blue}{\text{40}}$ (New)
- `max4`: $\color{gray}{\text{200}}$ (Old) &rarr; $\color{blue}{\text{40}}$ (New)
- `prop7`: $\color{gray}{\text{dmg-norm}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min7`: $\color{gray}{\text{50}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)
- `max7`: $\color{gray}{\text{50}}$ (Old) &rarr; $\color{blue}{\text{0}}$ (New)

### the corrupted grandfather
- `*ID`: $\color{gray}{\text{426}}$ (Old) &rarr; $\color{blue}{\text{464}}$ (New)
- `min2`: $\color{gray}{\text{250}}$ (Old) &rarr; $\color{blue}{\text{300}}$ (New)
- `max2`: $\color{gray}{\text{250}}$ (Old) &rarr; $\color{blue}{\text{350}}$ (New)
- `min4`: $\color{gray}{\text{150}}$ (Old) &rarr; $\color{blue}{\text{250}}$ (New)
- `max4`: $\color{gray}{\text{150}}$ (Old) &rarr; $\color{blue}{\text{250}}$ (New)
- `min7`: $\color{gray}{\text{175}}$ (Old) &rarr; $\color{blue}{\text{200}}$ (New)
- `max7`: $\color{gray}{\text{175}}$ (Old) &rarr; $\color{blue}{\text{200}}$ (New)
- `min8`: $\color{gray}{\text{25}}$ (Old) &rarr; $\color{blue}{\text{40}}$ (New)
- `max8`: $\color{gray}{\text{25}}$ (Old) &rarr; $\color{blue}{\text{50}}$ (New)
- `min9`: $\color{gray}{\text{25}}$ (Old) &rarr; $\color{blue}{\text{40}}$ (New)
- `max9`: $\color{gray}{\text{25}}$ (Old) &rarr; $\color{blue}{\text{50}}$ (New)
- `prop10`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{pierce-dmg}}$ (New)
- `min10`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{15}}$ (New)
- `max10`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{30}}$ (New)

### the gnasher
- `prop7`: $\color{gray}{\text{dmg-norm}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min7`: $\color{gray}{\text{20}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `max7`: $\color{gray}{\text{20}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)

### the grandfather
- `max1`: $\color{gray}{\text{20}}$ (Old) &rarr; $\color{blue}{\text{30}}$ (New)
- `prop2`: $\color{gray}{\text{att\%}}$ (Old) &rarr; $\color{blue}{\text{pierce-dmg}}$ (New)
- `min2`: $\color{gray}{\text{50}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `max2`: $\color{gray}{\text{50}}$ (Old) &rarr; $\color{blue}{\text{20}}$ (New)
- `min3`: $\color{gray}{\text{80}}$ (Old) &rarr; $\color{blue}{\text{100}}$ (New)
- `max3`: $\color{gray}{\text{80}}$ (Old) &rarr; $\color{blue}{\text{100}}$ (New)

### thunderfury
- `*ID`: $\color{gray}{\text{416}}$ (Old) &rarr; $\color{blue}{\text{454}}$ (New)

### thunderlords overcharge
- `*ID`: $\color{gray}{\text{432}}$ (Old) &rarr; $\color{blue}{\text{470}}$ (New)

### trophy of the lichking toudi
- `*ID`: $\color{gray}{\text{453}}$ (Old) &rarr; $\color{blue}{\text{491}}$ (New)

### tyrael's might
- `prop9`: $\color{gray}{\text{swing2}}$ (Old) &rarr; $\color{blue}{\text{oskill}}$ (New)
- `par9`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{Teleport}}$ (New)
- `min9`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)
- `max9`: $\color{gray}{\text{30}}$ (Old) &rarr; $\color{blue}{\text{1}}$ (New)
- `prop10`: $\color{gray}{\text{oskill}}$ (Old) &rarr; $\color{blue}{\text{allskills}}$ (New)
- `par10`: $\color{gray}{\text{Teleport}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min10`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{2}}$ (New)
- `max10`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{2}}$ (New)
- `prop11`: $\color{gray}{\text{allskills}}$ (Old) &rarr; $\color{blue}{\text{swing2}}$ (New)
- `min11`: $\color{gray}{\text{2}}$ (Old) &rarr; $\color{blue}{\text{30}}$ (New)
- `max11`: $\color{gray}{\text{2}}$ (Old) &rarr; $\color{blue}{\text{30}}$ (New)

### undeniable
- `*ID`: $\color{gray}{\text{450}}$ (Old) &rarr; $\color{blue}{\text{488}}$ (New)
- `firstLadderSeason`: $\color{gray}{\text{Magic Conviction}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `par1`: $\color{gray}{\text{376}}$ (Old) &rarr; $\color{blue}{\text{431}}$ (New)

### unsullied's glory
- `*ID`: $\color{gray}{\text{406}}$ (Old) &rarr; $\color{blue}{\text{444}}$ (New)

### unsullied's legacy
- `*ID`: $\color{gray}{\text{407}}$ (Old) &rarr; $\color{blue}{\text{445}}$ (New)

### viperfork
- `prop4`: $\color{gray}{\text{get-hit-skill}}$ (Old) &rarr; $\color{blue}{\text{gethit-skill}}$ (New)

### wind god fist
- `*ID`: $\color{gray}{\text{434}}$ (Old) &rarr; $\color{blue}{\text{472}}$ (New)

### windhammer
- `par6`: $\color{gray}{\text{tornado}}$ (Old) &rarr; $\color{blue}{\text{Tornado}}$ (New)
- `prop7`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{pierce-dmg}}$ (New)
- `min7`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `max7`: $\color{gray}{\text{*empty*}}$ (Old) &rarr; $\color{blue}{\text{15}}$ (New)

### woestave
- `prop9`: $\color{gray}{\text{aura}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `par9`: $\color{gray}{\text{Meditation}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `min9`: $\color{gray}{\text{12}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)
- `max9`: $\color{gray}{\text{17}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)

### wraithflight
- `max1`: $\color{gray}{\text{250}}$ (Old) &rarr; $\color{blue}{\text{240}}$ (New)

### wrath of the high heavens
- `*ID`: $\color{gray}{\text{423}}$ (Old) &rarr; $\color{blue}{\text{461}}$ (New)

