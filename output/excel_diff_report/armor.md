# Differences for armor.txt

*Key column used: `name`*

## Added Columns in BK (New)
`DropConditionCalc, UICatOverride`  

## Added Rows in BK (New) (15)
- blasphemous compendium
- blasphemous grimoire
- burnt text
- codex
- compendium
- dark codex
- dark tome
- forgotten volume
- grimoire
- occult codex
- occult tome
- old book
- possessed compendium
- possessed grimoire
- tome

## Modified Rows (69)
### aegis
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)

### ancient armor
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{226}}$ (Old) &rarr; $\color{blue}{\text{426}}$ (New)

### archon plate
- `auto prefix`: $\color{gray}{\text{218}}$ (Old) &rarr; $\color{blue}{\text{418}}$ (New)

### balrog skin
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{221}}$ (Old) &rarr; $\color{blue}{\text{421}}$ (New)

### bloodlord skull
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)

### boneweave
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{220}}$ (Old) &rarr; $\color{blue}{\text{420}}$ (New)

### breast plate
- `auto prefix`: $\color{gray}{\text{216}}$ (Old) &rarr; $\color{blue}{\text{416}}$ (New)

### cantor trophy
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### chain mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{220}}$ (Old) &rarr; $\color{blue}{\text{420}}$ (New)

### chaos armor
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `auto prefix`: $\color{gray}{\text{225}}$ (Old) &rarr; $\color{blue}{\text{425}}$ (New)

### cuirass
- `auto prefix`: $\color{gray}{\text{216}}$ (Old) &rarr; $\color{blue}{\text{416}}$ (New)

### demon head
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### demonhide armor
- `auto prefix`: $\color{gray}{\text{214}}$ (Old) &rarr; $\color{blue}{\text{414}}$ (New)

### diamond mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{217}}$ (Old) &rarr; $\color{blue}{\text{417}}$ (New)

### dusk shroud
- `auto prefix`: $\color{gray}{\text{212}}$ (Old) &rarr; $\color{blue}{\text{412}}$ (New)

### embossed plate
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{224}}$ (Old) &rarr; $\color{blue}{\text{424}}$ (New)

### expansion
- `ShowLevel`: $\color{gray}{\text{1}}$ (Old) &rarr; $\color{blue}{\text{*empty*}}$ (New)

### fetish trophy
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### field plate
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{222}}$ (Old) &rarr; $\color{blue}{\text{422}}$ (New)

### full plate mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `auto prefix`: $\color{gray}{\text{225}}$ (Old) &rarr; $\color{blue}{\text{425}}$ (New)

### gargoyle head
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### ghost armor
- `auto prefix`: $\color{gray}{\text{212}}$ (Old) &rarr; $\color{blue}{\text{412}}$ (New)

### gothic plate
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `reqstr`: $\color{gray}{\text{100}}$ (Old) &rarr; $\color{blue}{\text{70}}$ (New)
- `auto prefix`: $\color{gray}{\text{224}}$ (Old) &rarr; $\color{blue}{\text{424}}$ (New)

### gothic shield
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)

### great hauberk
- `auto prefix`: $\color{gray}{\text{216}}$ (Old) &rarr; $\color{blue}{\text{416}}$ (New)

### hard leather armor
- `auto prefix`: $\color{gray}{\text{214}}$ (Old) &rarr; $\color{blue}{\text{414}}$ (New)

### heirophant trophy
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### hellforge plate
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `auto prefix`: $\color{gray}{\text{223}}$ (Old) &rarr; $\color{blue}{\text{423}}$ (New)

### hellspawn skull
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)

### hyperion
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)

### kraken shell
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{222}}$ (Old) &rarr; $\color{blue}{\text{422}}$ (New)

### lacquered plate
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{224}}$ (Old) &rarr; $\color{blue}{\text{424}}$ (New)

### large shield
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)

### leather armor
- `auto prefix`: $\color{gray}{\text{213}}$ (Old) &rarr; $\color{blue}{\text{413}}$ (New)

### light plate
- `auto prefix`: $\color{gray}{\text{218}}$ (Old) &rarr; $\color{blue}{\text{418}}$ (New)

### linked mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{217}}$ (Old) &rarr; $\color{blue}{\text{417}}$ (New)

### loricated mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `auto prefix`: $\color{gray}{\text{219}}$ (Old) &rarr; $\color{blue}{\text{419}}$ (New)

### mage plate
- `auto prefix`: $\color{gray}{\text{218}}$ (Old) &rarr; $\color{blue}{\text{418}}$ (New)

### mesh armor
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{220}}$ (Old) &rarr; $\color{blue}{\text{420}}$ (New)

### minion skull
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)

### mummified trophy
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### ornate armor
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{226}}$ (Old) &rarr; $\color{blue}{\text{426}}$ (New)

### overseer skull
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)

### pavise
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)

### plate mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `auto prefix`: $\color{gray}{\text{223}}$ (Old) &rarr; $\color{blue}{\text{423}}$ (New)

### preserved head
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### quilted armor
- `auto prefix`: $\color{gray}{\text{212}}$ (Old) &rarr; $\color{blue}{\text{412}}$ (New)

### ring mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{217}}$ (Old) &rarr; $\color{blue}{\text{417}}$ (New)

### russet armor
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{221}}$ (Old) &rarr; $\color{blue}{\text{421}}$ (New)

### sacred armor
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{226}}$ (Old) &rarr; $\color{blue}{\text{426}}$ (New)
- `gemsockets`: $\color{gray}{\text{6}}$ (Old) &rarr; $\color{blue}{\text{4}}$ (New)

### scale mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `auto prefix`: $\color{gray}{\text{219}}$ (Old) &rarr; $\color{blue}{\text{419}}$ (New)

### scarab husk
- `auto prefix`: $\color{gray}{\text{214}}$ (Old) &rarr; $\color{blue}{\text{414}}$ (New)

### scutum
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)

### serpentskin armor
- `auto prefix`: $\color{gray}{\text{213}}$ (Old) &rarr; $\color{blue}{\text{413}}$ (New)

### sexton trophy
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### shadow plate
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `auto prefix`: $\color{gray}{\text{225}}$ (Old) &rarr; $\color{blue}{\text{425}}$ (New)

### sharktooth armor
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{222}}$ (Old) &rarr; $\color{blue}{\text{422}}$ (New)

### splint mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)
- `auto prefix`: $\color{gray}{\text{221}}$ (Old) &rarr; $\color{blue}{\text{421}}$ (New)

### studded leather
- `auto prefix`: $\color{gray}{\text{215}}$ (Old) &rarr; $\color{blue}{\text{415}}$ (New)

### succubus skull
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)

### templar coat
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `auto prefix`: $\color{gray}{\text{223}}$ (Old) &rarr; $\color{blue}{\text{423}}$ (New)

### tigulated mail
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)
- `auto prefix`: $\color{gray}{\text{219}}$ (Old) &rarr; $\color{blue}{\text{419}}$ (New)

### tower shield
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{10}}$ (New)

### trellised armor
- `auto prefix`: $\color{gray}{\text{215}}$ (Old) &rarr; $\color{blue}{\text{415}}$ (New)

### unraveller head
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

### ward
- `speed`: $\color{gray}{\text{0}}$ (Old) &rarr; $\color{blue}{\text{5}}$ (New)

### wire fleece
- `auto prefix`: $\color{gray}{\text{215}}$ (Old) &rarr; $\color{blue}{\text{415}}$ (New)

### wyrmhide
- `auto prefix`: $\color{gray}{\text{213}}$ (Old) &rarr; $\color{blue}{\text{413}}$ (New)

### zombie head
- `auto prefix`: $\color{gray}{\text{305}}$ (Old) &rarr; $\color{blue}{\text{307}}$ (New)
- `gemsockets`: $\color{gray}{\text{4}}$ (Old) &rarr; $\color{blue}{\text{3}}$ (New)

