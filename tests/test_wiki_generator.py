...
class TestWikiGenerator(unittest.TestCase):
    ...
    def test_templates_render_item_and_index_pages(self):
        ...
        self.assertIn("data-runeword-filters", runewords_index_page)
        ...
        self.assertIn('data-runeword-title="Practice"', runewords_index_page)
        self.assertIn('data-base-items="Helm"', runewords_index_page)
        self.assertIn('data-runes="Tal Rune|Tal|r07|Eth Rune|Eth|r05"', runewords_index_page)
        self.assertIn('data-rune-codes="r07|r05"', runewords_index_page)
        self.assertNotIn("Merc Equip", runewords_index_page)
        self.assertRegex(runewords_index_page, r'data-search="[^"]*Holy Shock[^"]*"')
        self.assertIn("function wireRunewordFilters()", site_js)
        self.assertIn('normalizeText(card.dataset.runewordTitle || "").startsWith(prefix)', site_js)
        self.assertIn("baseItems.includes(normalizedBase)", site_js)
        self.assertIn("runeCodes.every((code) => selectedRuneCodes.has(code))", site_js)
        self.assertIn(".rune-option.is-selected", site_css)
