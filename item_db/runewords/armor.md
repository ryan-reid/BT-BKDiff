# Runewords for Armor

### Authority
* **Runes:** Hel Rune + Shael Rune + Ral Rune
* **Base Items:** tors
* **Properties:**
    * 2-10% Chance to cast level %d %s when struck
    * 10% Chance to cast Level 15 Miasma Chains on striking
    * +2 to Amazon Skill Levels
    * : 40-60 (param: )

### Bone
* **Runes:** Sol Rune + Amn Rune + Amn Rune
* **Base Items:** tors
* **Properties:**
    * +15-20%% Faster Cast Rate
    * 15-10% Chance to cast level %d %s when struck
    * +2 to Amazon Skill Levels
    * +100-150 to Mana
    * Fire Resist +15%%

### Bramble
* **Runes:** Ral Rune + Ohm Rune + Sur Rune + Eth Rune
* **Base Items:** tors
* **Properties:**
    * +50%% Faster Hit Recovery
    * +300 Defense
    * Level 15 Thorns Aura When Equipped
    * +13 Life after each Kill
    * +25-50%% to Poison Skill Damage
    * Poison Resist +100%%
    * Level 33-13 %s (33-13/33-13 Charges)

### Broken Promise
* **Runes:** Io Rune + Lum Rune + Fal Rune + Ko Rune
* **Base Items:** tors
* **Properties:**
    * +20%% Faster Hit Recovery
    * +20%% Faster Run/Walk
    * +10 to Strength
    * +20%% Faster Cast Rate
    * Cannot Be Frozen
    * +20%% Increased Attack Speed
    * 20% Better Chance of Getting Magic Items

### Chains of Honor
* **Runes:** Dol Rune + Um Rune + Ber Rune + Ist Rune
* **Base Items:** tors
* **Properties:**
    * Fire Resist +50%%
    * +70%% Enhanced Defense
    * +200%% Damage to Demons
    * +100%% Damage to Undead
    * 8% Life stolen per hit
    * +2 to All Skills
    * +20 to Strength

### Dragon
* **Runes:** Sur Rune + Lo Rune + Sol Rune
* **Base Items:** tors
* **Properties:**
    * +360 Defense
    * +230 Defense vs. Missile
    * + to Strength (Based on Character Level)
    * 12% Chance to cast Level 15 Hydra on striking
    * 20-18% Chance to cast level %d %s when struck
    * Level 16 Holy Fire Aura When Equipped
    * +5-10 to Strength

### Duress
* **Runes:** Shael Rune + Um Rune + Thul Rune
* **Base Items:** tors
* **Properties:**
    * +15-20 to Mana after each Kill
    * : 80-100 (param: )
    * +150-200%% Enhanced Defense
    * +20%% Faster Hit Recovery
    * +33%% Chance of Open Wounds
    * +15%% Chance of Crushing Blow
    * +9-15 Life after each Kill

### Enigma
* **Runes:** Jah Rune + Ith Rune + Sol Rune
* **Base Items:** tors
* **Properties:**
    * +750-775 Defense
    * +14 Life after each Kill
    * +45%% Faster Run/Walk
    * +2 to All Skills
    * +1 to Teleport
    * % Better Chance of Getting Magic Items (Based on Character Level)

### Enlightenment
* **Runes:** Pul Rune + Ral Rune + Sol Rune
* **Base Items:** tors
* **Properties:**
    * +15-20%% Faster Cast Rate
    * 5-15% Chance to cast level %d %s when struck
    * +2 to Amazon Skill Levels
    * +1 to Warmth
    * +20%% Faster Hit Recovery

### Gloom
* **Runes:** Fal Rune + Um Rune + Pul Rune
* **Base Items:** tors
* **Properties:**
    * +170-230%% Enhanced Defense
    * Fire Resist +30%%
    * 15-6% Chance to cast level %d %s when struck
    * +23%% Faster Hit Recovery
    * +5%% Damage Taken Goes To Mana
    * 100-55% Chance to cast level %d %s when you Die
    * Half Freeze Duration

### Lionheart
* **Runes:** Hel Rune + Lum Rune + Fal Rune
* **Base Items:** tors
* **Properties:**
    * +15 to Strength
    * +20 to Vitality
    * +15 to Dexterity
    * : 90-110 (param: )
    * +50 to Life
    * Fire Resist +30%%

### Myth
* **Runes:** Hel Rune + Amn Rune + Nef Rune
* **Base Items:** tors
* **Properties:**
    * 10% Chance to cast Level 1 Taunt on striking
    * +20%% Faster Run/Walk
    * +2 to Amazon Skill Levels
    * Replenish Life +10

### Peace
* **Runes:** Shael Rune + Thul Rune + Amn Rune
* **Base Items:** tors
* **Properties:**
    * 2% Chance to cast Level 15 Valkyrie on striking
    * 4-5% Chance to cast level %d %s when struck
    * +2 to Amazon Skill Levels
    * +2 to Critical Strike

### Pestilence
* **Runes:** Tal Rune + Cham Rune + Ist Rune
* **Base Items:** tors
* **Properties:**
    * +3 to Poison Skills
    * -40-50% to Enemy Poison Resistance
    * Prevent Monster Heal
    * +400-500 Defense
    * 50% Chance to cast level %d %s when struck
    * Level 15 cleansing Aura When Equipped
    * Magic Resist +50%%

### Principle
* **Runes:** Ral Rune + Ko Rune + Eld Rune
* **Base Items:** tors
* **Properties:**
    * 100% Chance to cast Level 5 Holy Bolt on striking
    * +2 to Amazon Skill Levels
    * +100-150 to Life
    * +50%% Damage to Undead

### Prudence
* **Runes:** Mal Rune + Tir Rune
* **Base Items:** tors
* **Properties:**
    * +140-170%% Enhanced Defense
    * Magic Damage Reduced by 17
    * Damage Reduced by 24
    * Fire Resist +25-35%%
    * +25%% Faster Hit Recovery
    * Repairs  durability per second
    * Cannot Be Frozen

### Rain
* **Runes:** Ort Rune + Dol Rune + Ith Rune
* **Base Items:** tors
* **Properties:**
    * +15-20%% Faster Cast Rate
    * 5-15% Chance to cast level %d %s when struck
    * +2 to Amazon Skill Levels
    * +100-150 to Mana

### Red
* **Runes:** Io Rune + Eld Rune + Dol Rune
* **Base Items:** tors
* **Properties:**
    * Increase Maximum Life 15-25%
    * +80-100 to Life
    * 9% Life stolen per hit
    * +5-10 Life after each Kill
    * Fire Resist +75%%
    * Fire Absorb +20%%
    * +50-80%% Enhanced Defense

### Smoke
* **Runes:** Nef Rune + Lum Rune
* **Base Items:** tors
* **Properties:**
    * +250 Defense vs. Missile
    * +75%% Enhanced Defense
    * Fire Resist +50%%
    * +20%% Faster Hit Recovery
    * +3-5 to Strength (Based on Character Level)
    * 20-13% Chance to cast level %d %s when you Kill an Enemy

### Stealth
* **Runes:** Tal Rune + Eth Rune
* **Base Items:** tors
* **Properties:**
    * Magic Damage Reduced by 3
    * +6 to Dexterity
    * +15 Maximum Stamina
    * +25%% Faster Run/Walk
    * +25%% Faster Cast Rate
    * +25%% Faster Hit Recovery

### Stone
* **Runes:** Shael Rune + Um Rune + Pul Rune + Lum Rune
* **Base Items:** tors
* **Properties:**
    * +350-400%% Enhanced Defense
    * +30 to Clay Golem
    * +300 Defense vs. Missile
    * +10-15%% Strength
    * +15-20 to Strength
    * +15-20 to Vitality
    * +40%% Faster Hit Recovery

### Treachery
* **Runes:** Shael Rune + Thul Rune + Lem Rune
* **Base Items:** tors
* **Properties:**
    * 25% Chance to cast Level 15 Venom on striking
    * 50-15% Chance to cast level %d %s when struck
    * +2 to Amazon Skill Levels
    * +45%% Increased Attack Speed

### Wealth
* **Runes:** Lem Rune + Ko Rune + Tir Rune
* **Base Items:** tors
* **Properties:**
    * 250% Extra Gold from Monsters
    * % Better Chance of Getting Magic Items (Based on Character Level)
    * +20%% Faster Run/Walk
    * +10%% Increased Attack Speed

### Hustle (armor)
* **Runes:** Shael Rune + Ko Rune + Eld Rune
* **Base Items:** tors
* **Properties:**
    * +65%% Faster Run/Walk
    * 35% Slower Stamina Drain
    * +40%% Increased Attack Speed
    * Fire Resist +10%%
    * +6 to Evade

### Dilemma
* **Runes:** Dol Rune + Lem Rune + Mal Rune
* **Base Items:** tors
* **Properties:**
    * +1 to Teleport
    * Drain Life -40--20
    * -35--25 Life after each Kill
    * 35-50% Better Chance of Getting Magic Items

### Shadows
* **Runes:** Shael Rune + Sol Rune + Ko Rune
* **Base Items:** tors
* **Properties:**
    * +45%% Increased Attack Speed
    * +100-150 Kick Damage
    * +100-150 to Mana
    * +2 to Amazon Skill Levels
    * +5%% Deadly Strike
    * Cold Resist +30%%
