# Unique Amulet

### Nokozan Relic (Nokozan Relic)
* **Base Item:** Amulet
* **Level Requirement:** 10
* **Properties:**
    * Adds 50-100-50-100 Fire Damage
    * +10% to Maximum Fire Resist
    * Fire Resist +50%
    * +20% Faster Hit Recovery
    * Fire Absorb 20%

### The Eye of Etlich (The Eye of Etlich)
* **Base Item:** Amulet
* **Level Requirement:** 15
* **Properties:**
    * +10-40 Defense vs. Missile
    * +1-2 to Cold Skills
    * 3-7% Life stolen per hit
    * -8-15% to Enemy Cold Resistance
    * Adds 100-200-100-200 Cold Damage

### The Mahim-Oak Curio (The Mahim-Oak Curio)
* **Base Item:** Amulet
* **Level Requirement:** 25
* **Properties:**
    * +10 to Dexterity
    * +10 to Strength
    * +10 to Energy
    * +10 to Vitality
    * +10 Defense
    * 10% Bonus to Attack Rating
    * All Resistances +10
    * +10% Enhanced Defense

### Amulet of the Viper (Amulet of the Viper)
* **Base Item:** Top of the Horadric Staff
* **Level Requirement:** 0
* **Properties:**
    * +10 to Mana
    * Poison Resist +25%
    * +10 to Life

### The Cat's Eye (The Cat's Eye)
* **Base Item:** Amulet
* **Level Requirement:** 50
* **Properties:**
    * +30% Faster Run/Walk
    * +20% Increased Attack Speed
    * +100 Defense
    * +100 Defense vs. Missile
    * +25 to Dexterity
    * +100 Defense vs. Melee

### The Rising Sun (The Rising Sun)
* **Base Item:** Amulet
* **Level Requirement:** 65
* **Properties:**
    * Absorbs Fire Damage (Based on Character Level)
    * 50-40% Chance to cast level 50-40 Fire Ball when struck
    * Adds 100-200-100-200 Fire Damage
    * +2 to Fire Skills
    * Replenish Life +10
    * +15-20% to Fire Skill Damage

### Crescent Moon (Crescent Moon)
* **Base Item:** Amulet
* **Level Requirement:** 50
* **Properties:**
    * 11-15% Mana stolen per hit
    * Magic Damage Reduced by 10
    * 10% Damage Taken Goes To Mana
    * +45 to Mana
    * 3-6% Life stolen per hit
    * +2 to Lightning Skills
    * +15% Increased Attack Speed
    * +15% Faster Cast Rate

### Mara's Kaleidoscope (Mara's Kaleidoscope)
* **Base Item:** Amulet
* **Level Requirement:** 67
* **Properties:**
    * +2 to All Skills
    * All Resistances +20-30
    * +5 to Strength
    * +5 to Dexterity
    * +5 to Vitality
    * +5 to Energy

### Atma's Scarab (Atma's Scarab)
* **Base Item:** Amulet
* **Level Requirement:** 60
* **Properties:**
    * Adds 102-102 Poison Damage Over 102 Seconds
    * Poison Resist +75%
    * Attacker Takes Damage of 120
    * 25-22% Chance to cast level 25-22 Amplify Damage on striking
    * 20% Bonus to Attack Rating

### Highlord's Wrath (Highlord's Wrath)
* **Base Item:** Amulet
* **Level Requirement:** 65
* **Properties:**
    * Lightning Resist +35%
    * Adds 1-30-1-30 Lightning Damage
    * +20% Increased Attack Speed
    * +1 to All Skills
    * % Deadly Strike (Based on Character Level)
    * Attacker Takes Lightning Damage of 15

### Saracen's Chance (Saracen's Chance)
* **Base Item:** Amulet
* **Level Requirement:** 47
* **Properties:**
    * All Resistances +35-40
    * 24-20% Chance to cast level 24-20 Lower Resist when struck
    * +20-30 to all Attributes
    * +1 to Random Class Skill Levels

### Seraph's Hymn (Seraph's Hymn)
* **Base Item:** Amulet
* **Level Requirement:** 65
* **Properties:**
    * +2 to All Skills
    * +1-2 to Defensive Auras Skills
    * +50-100% Damage to Demons
    * +50-100% Damage to Undead
    * +150-250 to Attack Rating against Demons
    * +150-250 to Attack Rating against Undead

### Metalgrid (Metalgrid)
* **Base Item:** Amulet
* **Level Requirement:** 81
* **Properties:**
    * +300-350 Defense
    * All Resistances +25-35
    * +400-450 to Attack Rating
    * +20 to Iron Golem
    * +20 to Summon Resist
    * +20 to Golem Mastery

### Entropy Locket (Entropy Locket)
* **Base Item:** Amulet
* **Level Requirement:** 54
* **Properties:**
    * 20-25% Chance to cast level 20-25 Miasma Chain on striking
    * +5-10% to Magic Skill Damage
    * +10% Faster Cast Rate
    * Lightning Resist +25-40%
    * Increase Maximum Mana 10-15%
    * Magic Damage Reduced by 8-12

### Corrupted Saracen's Chance (Corrupted Saracen's Chance)
* **Base Item:** Amulet
* **Level Requirement:** 47
* **Properties:**
    * 10-5% Chance to cast level 10-5 Amplify Damage on striking
    * 1-63% Chance to cast level 1-63 Iron Maiden when struck
    * +10 to Strength
    * +10 to Dexterity
    * All Resistances +15
