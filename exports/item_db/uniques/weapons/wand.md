# Unique Wand

### Torch of Iro (Iros Torch)
* **Base Item:** Wand
* **Level Requirement:** 5
* **Properties:**
    * +1-2 to All Skills
    * +100 to Mana
    * +5 to Warmth
    * +10 to Energy
    * Regenerate Mana 25%
    * +50% Faster Run/Walk
    * +30% Faster Cast Rate

### Maelstrom (Maelstromwrath)
* **Base Item:** Yew Wand
* **Level Requirement:** 14
* **Properties:**
    * +1 to Minimum Lightning Damage
    * +100 to Maximum Lightning Damage
    * Lightning Resist +40%
    * +80 to Mana
    * +30% Faster Cast Rate
    * +1-3 to Corpse Explosion (Class only)
    * +1-3 to Bone Armor (Class only)
    * +1-3 to Amplify Damage (Class only)
    * +1-3 to Dim Vision (Class only)
    * +1-2 to Necromancer Skill Levels

### Gravenspine (Gravenspine)
* **Base Item:** Bone Wand
* **Level Requirement:** 20
* **Properties:**
    * +10 to Strength
    * +10 to Dexterity
    * +20% Faster Cast Rate
    * +20% Faster Run/Walk
    * +2-4 to Necromancer Skill Levels
    * +60 to Mana

### Ume's Lament (Umes Lament)
* **Base Item:** Grim Wand
* **Level Requirement:** 28
* **Properties:**
    * +2-3 to Necromancer Skill Levels
    * +80 to Mana
    * +40% Faster Cast Rate
    * +3 to Skeleton Mastery (Class only)
    * +2-3 to Decrepify (Class only)

### Suicide Branch (Suicide Branch)
* **Base Item:** Burnt Wand
* **Level Requirement:** 33
* **Properties:**
    * +10% Faster Hit Recovery
    * +70% Faster Cast Rate
    * All Resistances +-30
    * Increase Maximum Mana 1-20%
    * +1-50 to Life
    * +1-50 to Mana
    * Increase Maximum Life 1-5%
    * +1 to All Skills

### Carin Shard (Carin Shard)
* **Base Item:** Petrified Wand
* **Level Requirement:** 35
* **Properties:**
    * + to Life (Based on Character Level)
    * +30-40% Faster Cast Rate
    * + to Mana (Based on Character Level)
    * +30% Faster Hit Recovery
    * Replenish Life +30
    * +1-3 to Necromancer Skill Levels
    * +2-3 to Summoning Skills

### Arm of King Leoric (Arm of King Leoric)
* **Base Item:** Tomb Wand
* **Level Requirement:** 36
* **Properties:**
    * +1-4 to Summoning Skills
    * 15-40% Chance to cast level # Bone Spirit when struck
    * + to Mana (Based on Character Level)
    * 15-30% Chance to cast level # Bone Armor when struck
    * +30% Faster Cast Rate
    * +1-2 to Poison and Bone Skills
    * +2 to Corpse Explosion (Class only)
    * +2 to Summon Resist (Class only)
    * +3 to Skeleton Mastery (Class only)
    * +3 to Raise Skeleton (Class only)

### Blackhand Key (Blackhand Key)
* **Base Item:** Grave Wand
* **Level Requirement:** 41
* **Properties:**
    * +1-3 to Curses Skills
    * 20% Damage Taken Goes To Mana
    * +50 to Life
    * +30% Faster Cast Rate
    * Fire Resist +37%
    * +2 to Necromancer Skill Levels
    * +13 to Grim Ward

### Death's Web (Deaths's Web)
* **Base Item:** Unearthed Wand
* **Level Requirement:** 66
* **Properties:**
    * +2 to All Skills
    * -40-50% to Enemy Poison Resistance
    * +7-12 Life after each Kill
    * +7-12 to Mana after each Kill
    * +1-2 to Poison and Bone Skills
    * +20% Faster Cast Rate
    * 50-10% Chance to cast level # Lower Resist when you Kill an Enemy

### Boneshade (Boneshade)
* **Base Item:** Lich Wand
* **Level Requirement:** 79
* **Properties:**
    * +2 to Necromancer Skill Levels
    * +30% Faster Cast Rate
    * +4-5 to Teeth (Class only)
    * +4-5 to Bone Armor (Class only)
    * +2-3 to Bone Spear (Class only)
    * +1-2 to Bone Spirit (Class only)
    * +2-3 to Bone Wall (Class only)

### Resistbreaker (Resistbreaker)
* **Base Item:** Polished Wand
* **Level Requirement:** 79
* **Properties:**
    * Level 6-9 Magic Conviction Aura When Equipped
    * +2-4 to Magic Skills
    * +20-35% Faster Cast Rate
    * +20-30% to Magic Skill Damage
    * +10-20% Energy
    * +25 Magic Absorb

### Arcbane (Arcbane)
* **Base Item:** Ghost Wand
* **Level Requirement:** 81
* **Properties:**
    * Level 1-12 Magic Conviction Aura When Equipped
    * +1-2 to All Skills
    * +10-55% Faster Cast Rate
    * Curse Resist +50%
    * Piercing Attack
    * Increase Maximum Mana 10-20%

