# Unique Grimoire

### Ars Al'Diabolos (Ars Al'Diablolos)
* **Base Item:** Blasphemous Grimoire
* **Level Requirement:** 80
* **Properties:**
    * +2 to Warlock Skill Levels
    * 15% Chance to cast level 1 Terror when struck
    * +30% Faster Cast Rate
    * +170-200% Enhanced Defense
    * +15-25% to Fire Skill Damage
    * +5-10 to Mana after each Kill
    * Fire Resist +10-50%
    * +3-5 to Apocalypse (Class only)
    * +1-2 to Flame Wave (Class only)
    * +1-2 to Ring of Fire (Class only)
    * 10-25% Better Chance of Getting Magic Items

### Ars Tor'Baalos (Ars Tor'Baalos)
* **Base Item:** Blasphemous Compendium
* **Level Requirement:** 73
* **Properties:**
    * +4 to Warlock Skills
    * 15% Chance to cast level 1 Decrepify when struck
    * +1-2 to Demonic Mastery (Class only)
    * +1-2 to Blood Boil (Class only)
    * +1-2 to Engorge (Class only)
    * +1-2 to Consume (Class only)
    * +120-150% Enhanced Defense
    * +80-150 to Life
    * Damage Reduced by 5-15%
    * 10-25% Better Chance of Getting Magic Items

### Ars Dul'Mephistos (Ars Dul'Mephistos)
* **Base Item:** Occult Tome
* **Level Requirement:** 78
* **Properties:**
    * +2 to Warlock Skill Levels
    * 15% Chance to cast level 40 Blizzard when struck
    * +25-35% Faster Cast Rate
    * +30% Faster Hit Recovery
    * +100-200% Enhanced Damage
    * +1-3 to Psychic Ward (Class only)
    * +140-200% Enhanced Defense
    * -10-20% to Enemy Magic Resistance
    * 10-25% Better Chance of Getting Magic Items

### Measured Wrath (Measured Wrath)
* **Base Item:** Burnt Text
* **Level Requirement:** 52
* **Properties:**
    * +1-2 to Warlock Skill Levels
    * 30% Chance to cast level 25 Ring of Fire when struck
    * +25-30% Faster Cast Rate
    * +130-180% Enhanced Defense
    * +1-3 to Summon Tainted (Class only)
    * +1-3 to Ring of Fire (Class only)
    * +1-3 to Flame Wave (Class only)
    * +10-20 to Vitality
    * +3-5 Life after each Kill
    * All Resistances +20-30

