# Unique Primal Helm

### Arreat's Face (Arreat's Face)
* **Base Item:** Slayer Guard
* **Level Requirement:** 42
* **Properties:**
    * +2  to Barbarian Skill Levels
    * +2 to Combat Skills
    * +150-200% Enhanced Defense
    * +30% Faster Hit Recovery
    * 20% Bonus to Attack Rating
    * +20 to Strength
    * +20 to Dexterity
    * 3-6% Life stolen per hit
    * All Resistances +30

### Demonhorn's Edge (Demonhorn's Edge)
* **Base Item:** Destroyer Helm
* **Level Requirement:** 61
* **Properties:**
    * +120-160% Enhanced Defense
    * +20% Increased Attack Speed
    * 3-6% Life stolen per hit
    * Attacker Takes Damage of 55-77
    * +3-4 to Combat Skills
    * +3-4 to Combat Masteries Skills
    * +3-4 to Warcries Skills
    * +100-150% Damage to Demons

### Wolfhowl (Wolfhowl)
* **Base Item:** Fury Visor
* **Level Requirement:** 79
* **Properties:**
    * +120-150% Enhanced Defense
    * +2-3 to Warcries Skills
    * +10-15 to Strength
    * +10-15 to Dexterity
    * +10 to Vitality
    * +4-6 to Werewolf
    * +15 to Summon Dire Wolf
    * +4-6 to Lycanthropy
    * +4-6 to Feral Rage
    * +4-6 to Fury

### Halaberd's Reign (Halaberd's Reign)
* **Base Item:** Conqueror Crown
* **Level Requirement:** 77
* **Properties:**
    * +140-170% Enhanced Defense
    * +1 to Combat Masteries Skills
    * +2-3  to Barbarian Skill Levels
    * +20% Faster Hit Recovery
    * +15 Life after each Kill
    * +1-2 to Battle Orders (Class Battle Orders only)
    * +1-2 to Battle Command (Class Battle Command only)
    * Damage Reduced by 10-15%

### Bul Katho's Mind (Bul Katho's Mind)
* **Base Item:** Guardian Crown
* **Level Requirement:** 92
* **Properties:**
    * +1-2 to Warcries Skills
    * +2  to Barbarian Skill Levels
    * +20% Faster Cast Rate
    * +15-20 to all Attributes
    * +20% Faster Hit Recovery
    * % Better Chance of Getting Magic Items (Based on Character Level)
    * +150-200% Enhanced Defense
    * -5-10% to Enemy Physical Resistance

### Pride (Pride)
* **Base Item:** Jawbone Cap
* **Level Requirement:** 27
* **Properties:**
    * +1 to All Skills
    * +50% Enhanced Defense
    * +30% Enhanced Damage
    * +20% Faster Run/Walk
    * 10% Life stolen per hit
    * 10% Mana stolen per hit
